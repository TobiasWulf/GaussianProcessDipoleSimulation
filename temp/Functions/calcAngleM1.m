%===============================================================================
% phi = calcAngleM1(Bx,By,position,unitX,unitY)
...
% date    : 05.05.2017
%===============================================================================
function [phiBsum, phiSum] = calcAngleM1(Bx,By,position,unitX,unitY)
    ...
    phiSum = atan2d(By(positionY1,positionX1)*((1-p)*(1-q)) + By(positionY2,positionX2)*(p*(1-q)) + By(positionY3,positionX3)*(p*q) + By(positionY4,positionX4)*((1-p)*q),...
                 Bx(positionY1,positionX1)*((1-p)*(1-q)) + Bx(positionY2,positionX2)*(p*(1-q)) + Bx(positionY3,positionX3)*(p*q) + Bx(positionY4,positionX4)*((1-p)*q));
end