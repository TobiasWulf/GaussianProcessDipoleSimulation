...
% dataset was acquired and a data struct which contains the magnetic field
% resolution and the cosine and sine bridge images for variable Hx and Hy
% fieldstrengths. The raw dataset was acquired after the method Thorben Schüthe
% described in his IEEE paper for two-dimensional characterization of TMR 
% angular sensors. The sensor characterized for both bridges a cosine and sine
% bridge. The bridges have a physically phase shift of 90° so the sensor is able
% to reference a superimposed magnetic field in x- and y-direction. The
% field was generated by a cross coil setup.
%
% *The resulting TMR characterization field abstracts a full rotation for
% cosine and sine output voltages by representing one maximum and minimun in
% the characterization fields. So circular path on the charcterization fields
...
% amplitudes. The modulation frequency is  set to:
%
% $$f_m = 0.01Hz$$
%
...
% vectors. To fieldstrength images like down below the recorded data must
% be referenced backwards to driven stimulus of Hx- and Hy-direction. But
% at first the image size must be determined. Here fix size is set to
...
% point on the Hx- and Hy-axes and gets the record index of the stimulus as
...
% $$V_{cos}(H_x, H_y) = [mV/V]$$
% 
% The information of the image is built up in rows. Reference Hx for
...
% Hy and so on next histogram appends on the next row for the next Hy.
%
%
%% Sine Bridge Output
...
% references. The best results supports the "Rise" field because it has a wide
% linear plateau between -8.5 kA/m and 8.5 kA/m. So Rise characterization field
% is used in sensor array simulation. It is not needed to drive the sensor in
% saturation.
%
%
%% Dataset Structure
...
%
% * *MagneticField* - struct, contains Hx- and Hy-field vectors which are the
...
% built-in save function in a certain way to perform partial loads from the
% dataset.
%
...
% Edited on December 05. 2020 by Tobias Wulf: Add bidge gain for sensor outputs.
% Edited on December 05. 2020 by Tobias Wulf: Add Operating Point section.
% -->
% </html>
