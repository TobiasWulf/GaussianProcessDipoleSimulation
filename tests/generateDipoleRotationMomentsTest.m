% create full scale rotation with 0.5° resolution and no tilt, return moments
...
% create same rotation but only a subset of angles N = 7 with equal distances to
% each and another, return additionally index which reference to full scale
[M, t, idx] = generateDipoleRotationMoments(amp, 7, tilt, res);

% create shifted subset, shift by 22 positions in full scale theta, so with 0.5°
% resolution it is phase shift by 11°
